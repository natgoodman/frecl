...
## estimate hidden values from observed scores
doc_hiddn=function(frecl=NULL,version=1) {
  if (is.null(frecl)) frecl=read_frecl(version);
  num.hid=frecl[1,'total']-1;                   # number of hidden scores
  num.obs=nrow(frecl);                          # number of observed scores
  num.all=num.hid+num.obs;                    # number of scores in complete dataset
  ...
  BREAKPOINT('doc_hiddn: before failed consistency check for ties')
...
## like btr_obs but for unique values. also includes ties
ranku=function(frecl) {
  score=frecl$score;
  sapply(seq_along(score),function(i) {
    scoreu=unique(score[1:i]);
    length(which(scoreu>=score[i]));
  });
}  
...
TODO=function() {
  ...
  ## remember that smooth functions in stats_hiddn conflict with smoothers used by plotm
